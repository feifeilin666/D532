...
select distinct a.id,b.date,b.close_last from stock as a left join stock_data_raw
as b on a.tickersymbol=b.symbol
''')
#colnames=[x[0] for x in c.description]
...
select distinct a.id,b.id,1 from portfolio as a cross join stock as b where
tickersymbol in ('INFY','JKS')
''')
#colnames=[x[0] for x in c.description]
...
ui.div(ui.row(
ui.column(4,ui.input_selectize("x1","Select Stock" ,choices)), #Stock
...
def server(input, output, session):
    ...
    def current_portfolio():
        ...
        with reactive.isolate():
            if input.x3():
                ...
                params=[input.x1()]
                c.execute('''delete from stockportfolio where stockid = (select
                distinct id from stock where tickersymbol=?)''',params)
                c.execute('''select distinct e.tickersymbol,companyname,shares from
                user as a
                inner join portfolio as b on a.id=b.userid
                inner join stockportfolio as c on b.id=c.portfolioid
                inner join stockfact as d on c.stockid=d.stockid
                ...
                set shares = ? where stockid = (select distinct id from stock
                where tickersymbol= ?)''',params)
                c.execute('''select distinct e.tickersymbol,companyname,shares from
                user as a
                inner join portfolio as b on a.id=b.userid
                inner join stockportfolio as c on b.id=c.portfolioid
    ...
    def Stock_Dropdown_Trend():
        ...
        dropdown_df['date_time'] = pd.to_datetime(dropdown_df['date_time'],errors='coerce')
        ...
        dropdown_df2['date_time']=pd.to_datetime(dropdown_df2['date_time']).dt.to_period('m').astype(str)
        dropdown_df3= dropdown_df2.groupby(by=dropdown_df2['date_time'],as_index=False).agg({'stockprice':'mean'})
        #plt.plot(dropdown_df3.index.to_series().astype(str), dropdown_df3['stockprice'])
        plt.plot(dropdown_df3['date_time'], dropdown_df3['stockprice'])
        plt.xlabel("Date")
    ...
    def Portfolio_Trend():
        ...
        with reactive.isolate():
            ...
            port_trend_df.loc[:, 'year'] = pd.to_datetime(port_trend_df['date_time']).dt.year
            # Create yearly bins and calculate the average stock prices for each
